export async function linkedAds(slug, backendAPI, token, ads_table, message) {
  try {
    ...
    if (data.success) {
      ...
      setupUnlinkButtons(token, backendAPI, slug, message);
...
function setupUnlinkButtons(token, backendAPI, slug, message) {
        ...
        try {
          ...
          const data = await response.json();
            ...
            const newResponse = await fetch(
              backendAPI + `content-with-ads/${slug}`,
              {
            ...
            if (newData.success) {
              ...
              setupUnlinkButtons(token, backendAPI, slug, message);
                await updateAvailableAds(slug, token, backendAPI);
            }
          } else {
            alert(
...
export async function updateAvailableAds(slug, token, backendAPI) {
  try {
